...
names(envir)[-1]
# counting the variables except for the first one
...
summary(envir$Sites) #join with Sites column in common
#The output is numeric, but we don't want that
...
envir_coord <- merge(x = envir,
                     y = coord,
                     by = "Sites")
#Merging x column as envir, y column as coord, by the column in common Sites
#It's always useful to specify the column
# We can check the join with the `dim()` and `head()` functions. How many columns should we have at the end? What columns were added?
#It doesn't matter the class of the column, it is important to have a column in common.
...
# Check the dimensions in case of any repetition or loss of the data
...
#comm is not organized in a tidy form, so we need to convert it

# creating table with each species in each area species in rows
comm_df <- tidyr::pivot_longer(comm, cols = 2:ncol(comm), names_to = "TaxCode", values_to = "Abundance") #from n=2 column since in 1 we have the information of Sites

# Let's check the object's header and dimensions.
dim(comm_df)
head(comm_df)

#this is now tidy data
...
#comm_sp_join <- dplyr::left_join(comm_df,splitlist,) check record
...
colnames(traits)[1] <- "TaxCode" #Taking first name of the colname and assigning new name
